...
def average_slope_intercept(image,lines):
    ...
    right_ine=make_coordinates(image,right_fit_average)
    return np.array([left_line,right_ine])
...
canny_im=canny_image(lane_image)
cropped_image=region_of_interest(canny_im)
lines=cv2.HoughLinesP(cropped_image,2,np.pi/180,100,np.array([]),minLineLength=40,maxLineGap=5)
avrage_lines=average_slope_intercept(image,lines)
line_image=display_lines(lane_image,avrage_lines)
